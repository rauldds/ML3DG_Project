...
def colorize_sdf_mesh(sdf_generated_mesh,
    ...
    original_size_mesh = trimesh.load(
        "./results/mesh_input_cloud_watertight.obj")
    ...
    # trimesh.exchange.export.export_mesh(marching_cubes_mesh,"./results/mesh_input_cloud_recon_marching_cubes2.obj","obj")
    ...
    # trimesh.exchange.export.export_mesh(marching_cubes_mesh,"./results/mesh_input_cloud_recon_marching_cubes3.obj","obj")
    ...
    # trimesh.exchange.export.export_mesh(marching_cubes_mesh,"./results/mesh_input_cloud_recon_marching_cubes4.obj","obj")
    ...
    # trimesh.exchange.export.export_mesh(marching_cubes_mesh,"./results/mesh_input_cloud_recon_marching_cubes5.obj","obj")
    ...
    if GENERATE_TARGET:
        ...
        colorization_mesh_output_path = OUTPUT_DATASET_PATH +"/ScanObjectNNDataset_SDF_Colorize_Meshes/colorized_meshes/" + f"{class_name}/"
        ...
        trimesh.exchange.export.export_mesh(marching_cubes_mesh,
                                            OUTPUT_DATASET_PATH+"/ScanObjectNNDataset_SDF_Colorize_Meshes/colorized_meshes/"
                                            + f"{class_name}/" + file_name +".obj",
                                            "obj")
    else:
        colorization_mesh_output_path = OUTPUT_DATASET_PATH +"/InputSamples/colorized_meshes/" + f"{class_name}/{file_name}/"
        os.makedirs(colorization_mesh_output_path, exist_ok=True)
        trimesh.exchange.export.export_mesh(marching_cubes_mesh,
                                            OUTPUT_DATASET_PATH+"/InputSamples/colorized_meshes/"
                                            + f"{class_name}/{file_name}/" + f"{view_number}_{file_name}"+".obj",
                                            "obj")
...
def generate_sdf_npz_and_colorized_mesh(path_to_point_cloud: str,
    ...
    process = subprocess.Popen(['./additional_repos/Manifold/build/manifold'
    ...
    if GENERATE_TARGET:
        ...
        output_path = path_to_point_cloud.replace("object_dataset", "generated_dataset/ScanObjectNNDataset_SDF_Colorize_Meshes/SDFs")
        ...
        output_path = OUTPUT_DATASET_PATH+"/InputSamples/SDFs/" + f"{class_name}/{file_name}/" + f"{view_number}_{file_name}"
        shorted_path  = OUTPUT_DATASET_PATH+"/InputSamples/SDFs/" + f"{class_name}/{file_name}/"
        os.makedirs(shorted_path, exist_ok=True)
        np.savez(output_path, voxels)
        vertices, faces, normals, _ = skimage.measure.marching_cubes(voxels, level=0)
        sdf_generated_mesh = trimesh.Trimesh(vertices=vertices, faces=faces, vertex_normals=normals)
        # trimesh.exchange.export.export_mesh(sdf_generated_mesh,"./results/mesh_input_cloud_recon_marching_cubes.obj","obj")
